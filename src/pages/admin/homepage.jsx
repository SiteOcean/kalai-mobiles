...
export default function AdminHomePage (){
  ...
  const router = useRouter(null);
    ...
    const handleItemView=(itemId)=>{
      router.push('itemView?id='+itemId);
    }
    const deleteObject = async(id) => {
        const confirmDelete = window.confirm("Are you sure you want to delete this object?");
         ...
         try{
           const response = await deleteProductById(id,"deleteProductById")
          if(response){
            setProducts(
              products.filter((val) => {
                return val._id !== id;
    ...
    return(<LoginAuth>
      <div className='relative w-full'>
           ...
           {products ? <> {products.length > 0 ?<div className='grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-5 p-5'>
                 {products.map((val,i)=>{
                    return  <div key={i}  className="max-w-sm flex flex-col rounded relative overflow-hidden shadow-lg bg-white">
                    ...
                    <button className='bg-blue-500 px-2 py-1 rounded-b mt-1' onClick={()=>handleItemView(val._id)}>View</button>
                  </div>
                })} 
